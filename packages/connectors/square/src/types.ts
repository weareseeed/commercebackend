/**
 * Canonical imported-catalog model (weekly backlog item 8/9: "connector
 * abstraction + catalog import spike"). These types are the shared shape any
 * catalog connector (Square here; Shopify, BigCommerce, WooCommerce later)
 * maps its source-specific catalog format into, before it's written to
 * CommerceBackend's own `Listing` table. Only Square implements this so far,
 * but the shape itself is not Square-specific and is meant to be reused.
 */
