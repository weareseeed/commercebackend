/**
 * Canonical imported-catalog model (weekly backlog item 13: "BigCommerce
 * connector spike"). This is the same shared shape `packages/connectors/square`
 * and `packages/connectors/shopify` map their catalogs into — see those
 * packages' `types.ts` for the shape's origin. It is duplicated here rather
 * than imported from either package so no connector depends on another; a
 * future connector-neutral core package could extract this shape once it's
 * shared by enough connectors to be worth it.
 */
